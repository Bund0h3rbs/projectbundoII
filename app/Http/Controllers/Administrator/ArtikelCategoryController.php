...
{
    protected $folder = 'admin.artikel.kategori';
    ...
    public function index()
    {
        ...
        if ($cekAkses == true) {
            $data['title_header'] = "Kategori Artikel";
    ...
    function create(Request $request)
    {
        $datas = [];
        if($request->id){
            $datas = Artikel_category::find($request->id);
        }
        $with['data']  = $datas;
    ...
    function store(Request $request){
        ...
        $saveMenus = Artikel_category::find($request->id);
        if($saveMenus){
            $saveMenus->update($row);
        }else{
            Artikel_category::create($row);
        }
    ...
    function delete(Request $request){
        ...
        $delete = Artikel_category::find($request->id);
        if($delete){
            $delete->delete();
        }
